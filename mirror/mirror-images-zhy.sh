...
function create_ecr_repo() {
  ...
  then
    ...
    aws --profile=zhy --region ${ECR_REGION} ecr create-repository --repository-name "$1"    
    attach_policy "$1"
  fi
}
...
function attach_policy() {
  ...
  aws --profile zhy --region $ECR_REGION ecr  set-repository-policy --policy-text file://policy.text --repository-name "$1"
}
...
function is_remote_image_exists(){
  ...
  repo=${fullrepo%%:*}
  tag=${fullrepo##*:}
  res=$(aws --profile zhy --region $ECR_REGION ecr describe-images --repository-name "$repo" \
...
function ecr_login() {
  aws --profile=zhy ecr --region cn-northwest-1 get-login --no-include-email | sh
  aws --profile=zhy ecr --region cn-north-1 get-login --no-include-email | sh
...
all_ecr_repos=$(aws --profile=zhy --region $ECR_REGION ecr describe-repositories --query 'repositories[*].repositoryName' --output text)
echo "$all_ecr_repos"
